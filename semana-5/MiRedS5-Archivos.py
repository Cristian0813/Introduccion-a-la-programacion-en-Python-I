...
#1.✔️ Obtener datos del usuario
#2.✔️ Consultar y mostrar varios mensajes de estado del usuario
#3.✔️ Escoger entre distintas acciones que el usuario puede realizar
#4.✔️ Modificar el perfil del usuario
#
...
def leer_datos_usuario(nombre_archivo):
    ...
    if os.path.isfile(nombre_archivo):
        ...
        with open(nombre_archivo, "r") as archivo_usuario:
            ...
            nombre = limpiar_linea(archivo_usuario.readline())
            edad = int(limpiar_linea(archivo_usuario.readline()))
            estatura = float(limpiar_linea(archivo_usuario.readline()))
            estatura_m = int(estatura)
            estatura_cm = int( (estatura - estatura_m)*100 )
            sexo = limpiar_linea(archivo_usuario.readline())
            pais = limpiar_linea(archivo_usuario.readline())
            num_amigos = int(limpiar_linea(archivo_usuario.readline()))
            estado = limpiar_linea(archivo_usuario.readline())
        ...
        return None
#Llamamos a la función anterior pasandole el nombre completo del archivo
def escribir_datos_usuario(nombre_archivo, nombre, edad, estatura_m, estatura_cm, sexo, pais, num_amigos, estado):
        #que se guardará en formato .txt.
        with open(nombre_archivo, "w") as archivo_usuario:
            #Pedimos al usuario que ingrese sus datos personales
...
#Si hay un error de tipo IOError es porque el archivo no se puede abrir o escribir
nombre_archivo = nombre + ".user"
#La primera llamada a esta función crea el archivo y lo llena con los datos
...
if datos_usuario:
    ...
    print("El archivo de usuario no existe.")
...
while opcion != 0:
    ...
    if opcion == 1:
        ...
        escribir_datos_usuario(nombre_archivo, nombre, edad, estatura_m, estatura_cm, sexo, pais, num_amigos,estado)
        print("Archivo ",nombre_archivo,".user"," guardado")
...
#   y reducir el tamaÃ±o de tu cÃ³digo..lo
